...
class Eshidaipay extends ApiModel
{
    ...
    public static function getAllInfo($reqData, $payConf)
    {
        ...
        $data['shoporderId']  = $order;//商户订单号
        $data['totalamount']  = sprintf("%.2f", $amount);//金额
        $data['productname']  = 'good';//商品名/商品描述
        $data['remarks']      = 'thing';
        $data['storenumber']  = '123';
        $data['notify_url']   = $ServerUrl;
        $data['callback_url'] = $returnUrl;
        $data['nonce_str']    = self::nonceStr();//随机字符串;
        $signStr              = self::getSignStr($data, false, true);
        $data['pay_md5sign']  = strtoupper(self::getMd5Sign("{$signStr}", $payConf['md5_private_key']));
