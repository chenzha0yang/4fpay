...
class Shenzhoupay extends ApiModel
{
    ...
    public static function getAllInfo($reqData, $payConf)
    {
        ...
        //获取token
        $token_url = 'http://120.78.65.69:8100/api/auth/access-token';
        //第一次请求路径
    ...
    {
        libxml_disable_entity_loader(true);
        $dataInfo              = json_decode(json_encode(simplexml_load_string($request, 'SimpleXMLElement', LIBXML_NOCDATA)), true);
        $dataInfo['total_fee'] = $dataInfo['total_fee'] / 100;  //金额
        self::$resData         = $dataInfo;
        return $dataInfo;
    }

    public static function SignOther($type, $json, $payConf)
    {
        $dataInfo = self::$resData;
        $sign     = $dataInfo['sign'];
        unset($dataInfo['sign']);
        $signStr = self::getSignStr($dataInfo, true, true);
        $mysign  = strtoupper(md5($signStr . '&key=', $payConf['md5_private_key']));
        if ($dataInfo["status"] == '0' && $dataInfo['result_code'] == '0' && $sign == $mysign) {
            return true;
        } else {
            return false;
        }
