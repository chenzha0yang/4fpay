...
class Yonghengpay extends ApiModel
{
    ...
    public static function getAllInfo($reqData, $payConf)
    {
        ...
        $data['p3_Amt']          = sprintf('%.2f',$amount);//支付金额
        $data['p4_Cur']          = "CNY";//交易币种,固定值"CNY".
        $data['p5_Pid']          = 'goodsName';//商品名称
        $data['p6_Pcat']         = 'goodsType';//商品种类
        $data['p7_Pdesc']        = 'goodsDescribe';//商品描述
    ...
    public static function SignOther($model, $data, $payConf)
    {
        $sign = $data['hmac'];
        unset($data['hmac']);
        $mySign = self::HmacMd5($data,$payConf['md5_private_key']);
        if ($sign === $mySign && $data['r1_Code'] == 1) {
            return true;
        } else {
            return false;
        }
