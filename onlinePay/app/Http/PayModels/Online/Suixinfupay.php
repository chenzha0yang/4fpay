<?php

namespace App\Http\PayModels\Online;

use App\ApiModel;

class Suixinfupay extends ApiModel
{
    public static $method = 'post'; //提交方式 必加属性 post or get

    public static $reqType = 'form'; //提交类型 必加属性 form or curl or fileGet

    public static $payWay = 0; //是否需要生成二维码 必加属性 2 3 4 5

    public static $resType = 'json'; //curl file_get_contents 返回的数据类型json/xml/str

    public static $unit = 1; //金额单位  默认1为元  2:单位为分

    public static $postType = false; //数据提交类型 默认false 一维数组 or json/str/多维数组  getRequestByType

    public static $httpBuildQuery = false; //默认false/true为curl提交参数需要http_build_query

    public static $isAPP = false; // 判断是否跳转APP 默认false

    /**
     * @param array $reqData 接口传递的参数
     * @param array $payConf
     * @return array
     */
    public static function getAllInfo($reqData, $payConf)
    {
        /**
         * 参数赋值，方法间传递数组
         */
        $order     = $reqData['order'];
        $amount    = $reqData['amount'];
        $bank      = $reqData['bank'];
        $ServerUrl = $reqData['ServerUrl']; // 异步通知地址
        $returnUrl = $reqData['returnUrl']; // 同步通知地址

        //TODO: do something
        $data                = [];
        $data['pid']         = $payConf['business_num'];  // 商户号
        $data['bizOrderNo']  = $order;
        $data['createTime']  = date('Y-m-d H:i:s', time());
        $data['amount']      = sprintf("%.2f", $amount);  // 支付金额
        $data['returnUrl']   = $returnUrl;
        $data['notifyUrl']   = $ServerUrl;
        $data['key']         = $payConf['md5_private_key'];
        $signStr             = self::getSignStr($data, true, true);
        $sign                = strtoupper(self::getMd5Sign($signStr, ''));
        $data['sign']        = $sign;  // 签名
        $data['productCode'] = $bank ? $bank : '';
        if($payConf['is_app'] == 1){
            self::$isAPP = true;
        }
        self::$reqType       = 'curl';
        self::$resType       = 'other';
        unset($reqData);
        return $data;
    }

    public static function getQrCode($response)
    {
        $return = [];
        if ((int)$response['code'] === 0) {
            $return = json_decode($response['data'], true);
        } else {
            $return['code']    = $response['code'];
            $return['message'] = $response['message'];
        }
        return $return;
    }

    //回调处理
    public static function SignOther($type, $data, $payConf)
    {
        $sign    = strtoupper($data['sign']);
        unset($data['sign']);
        $data['key'] = $payConf['md5_private_key'];
        $signStr = self::getSignStr($data, false, true);
        $mySign  = strtoupper(self::getMd5Sign($signStr, ''));
        if ($mySign == $sign) {
            return true;
        } else {
            return false;
        }
    }
}