...
class Quancftwopay extends ApiModel
{
    ...
    public static function getAllInfo($reqData, $payConf)
    {
        ...
        $data['sign'] = self::sign($payConf['md5_private_key'], ['amount'=>$data['amount'],'out_trade_no'=>$data['out_trade_no']]);
        unset($reqData);
        return $data;
    }
    ...
    {
        $sign = $data['sign'];
        $signTrue = self::sign($payConf['md5_private_key'], ['amount'=>$data['amount'],'out_trade_no'=>$data['out_trade_no']]);
        if ( $data['account_key'] == $payConf['md5_private_key'] && $signTrue == $sign ) {
            return true;
        }
        return false;
    }
