...
class Haopay extends ApiModel
{
    ...
    public static function SignOther($model, $data, $payConf)
    {
        $sign = $data['sign'];
        unset($data['sign']);
        $signStr = "customerid=".$data['customerid']."&status=".$data['status']."&sdpayno=".$data['sdpayno']."&sdorderno=".$data['sdorderno']."&total_fee=".$data['total_fee']."&paytype=".$data['paytype']."&".$payConf['md5_private_key'];
        $mySign  = md5($signStr );
