...
class Lianshengpay extends ApiModel
{
    ...
    public static function getAllInfo($reqData, $payConf)
    {
        ...
        $data['type'] = $bank;                                          //银行类型
        $data['attach'] = 'protect';
        $data['value'] = sprintf("%.2f", $amount);                                        //金额
        $data['orderid'] = $order;//商户订单号
        $data['callbackurl'] = $ServerUrl;//下行异步通知地址
        $signSource                 = "parter=" . $payConf['business_num'] . "&type=" . $bank . "&value=" . $data['value'] . "&orderid=" . $order . "&callbackurl=" . $ServerUrl;
