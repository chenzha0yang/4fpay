<?php
namespace App\Http\PayModels\Online;

use App\ApiModel;

class Zhenhfpay extends ApiModel
{
    public static $method = 'post'; //提交方式 必加属性 post or get

    public static $reqType = 'form'; //提交类型 必加属性 form or curl or fileGet

    public static $payWay = 0; //是否需要生成二维码 必加属性 2 3 4 5

    public static $resType = ''; //curl file_get_contents 返回的数据类型json/xml/str

    public static $unit = 1; //金额单位  默认1为元  2:单位为分

    public static $httpBuildQuery = false; //默认 false  true为curl提交参数 需要http_build_query

    public static $postType = false; //数据提交类型 默认false 一维数组   or  json ／str ／多维数组

    public static $isAPP = false; // 判断是否跳转APP 默认false

    /**
     * @param array $reqData 接口传递的参数
     * @param array $payConf
     * @return array
     */
    public static function getAllInfo($reqData, $payConf)
    {
        /**
         * 参数赋值，方法间传递数组
         */
        $order     = $reqData['order'];
        $amount    = $reqData['amount'];
        $bank      = $reqData['bank'];
        $ServerUrl = $reqData['ServerUrl']; // 异步通知地址
        $returnUrl = $reqData['returnUrl']; // 同步通知地址

        if($payConf['pay_way'] != 1 ||  $payConf['pay_way'] != 9){
            self::$isAPP = true;
            self::$reqType = 'curl';
            self::$resType = 'other';
            self::$payWay = $payConf['pay_way'];
            self::$httpBuildQuery = true;
        }
        self::$unit = 2;
        //TODO: do something
        $data['nonceStr']    = self::randStr(); //随机字符串
        $data['startTime']   = date('YmdHis', time()); //订单生成时间
        $data['merchantNo']  = $payConf['business_num']; //商户号
        $data['outOrderNo']  = $order; //订单
        $data['amount']      = $amount * 100;
        $data['client_ip']   = self::getIp();
        $data['timestamp']   = time(); //时间戳
        $data['description'] = 'apple'; //商品描述
        $data['notifyUrl']   = $ServerUrl;
        if ($payConf['public_key'] == '1') { //收银台模式
            $data['returnUrl'] = $returnUrl; //同步地址
        }
        $data['extra']       = $order;
        $data['productCode'] = $bank; //支付方式
        if ($payConf['pay_way'] == '1') {
            $data['productCode'] = '1601';
            $data['bankCode']    = $bank;
        }
        $signStr      = self::getSignStr($data, true, true);
        $sign         = strtoupper(md5($signStr . '&key=' . $payConf['md5_private_key']));
        $data['sign'] = $sign; //签名
        unset($reqData);
        return $data;
    }

    /**
     * 二维码链接处理
     * @param $response
     * @return mixed
     */
    public static function getQrCode($response)
    {

        $data = json_decode($response,true);
        if($data['status'] == '200'){
            $data['payUrl'] = $data['data']['payUrl'];
        }

        return $data;
    }


    //随机数
    public static function randStr($length = 30)
    {
        $chars    = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ123456789';
        $password = '';
        for ($i = 0; $i < $length; $i++) {
            $password .= $chars[mt_rand(0, strlen($chars) - 1)];
        }
        return $password;
    }

    public static function getVerifyResult($request, $mod)
    {
        $data              = $request->all();
        $res['amount']     = $data['amount'] / 100;
        $res['outOrderNo'] = $data['outOrderNo'];
        return $res;
    }

     public static function SignOther($type, $json, $payConf)
    {
        $sign = $json['sign'];
        unset($json['sign']);
        $signStr      = self::getSignStr($json, true, true);
        $mysign         = strtoupper(md5($signStr . '&key=' . $payConf['md5_private_key']));
        if ($mysign == strtoupper($sign) && $json['orderStatus'] == '1') {
            return true;
        } else {
            return true;
        }
    }
}
