...
class Byszpay extends ApiModel
{
    ...
    public static function getVerifyResult($request, $mod)
    {
        ...
        $arr['user_order_no']  = $data['user_order_no'];
    ...
    public static function SignOther($mod, $data, $payConf)
    {
        ...
        $isSign = md5($data['user_order_no'].$data['orderno'].$data['tradeno'].$data['price'].$data['realprice'].$payConf['md5_private_key']);
        if ($data['sign'] == $isSign && $data['status'] == '3') {
            return true;
        }
        return false;
    }
