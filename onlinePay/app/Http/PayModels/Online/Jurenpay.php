<?php

namespace App\Http\PayModels\Online;

use App\ApiModel;

class Jurenpay extends ApiModel
{
    public static $method = 'post'; //提交方式 必加属性 post or get

    public static $reqType = 'form'; //提交类型 必加属性 form or curl or fileGet

    public static $payWay = 0; //是否需要生成二维码 必加属性 2 3 4 5

    public static $resType = 'json'; //curl file_get_contents 返回的数据类型json/xml/str

    public static $unit = 1; //金额单位  默认1为元  2:单位为分

    public static $httpBuildQuery = false; //默认 false  true为curl提交参数 需要http_build_query

    public static $postType = false; //数据提交类型 默认false 一维数组   or  json ／str ／多维数组

    public static $isAPP = false; // 判断是否跳转APP 默认false

    /**
     * @param array $reqData 接口传递的参数
     * @param array $payConf
     * @return array
     */
    public static function getAllInfo($reqData, $payConf)
    {
        /**
         * 参数赋值，方法间传递数组
         */
        $order     = $reqData['order'];
        $amount    = $reqData['amount'];
        $bank      = $reqData['bank'];
        $ServerUrl = $reqData['ServerUrl'];// 异步通知地址
        $returnUrl = $reqData['returnUrl'];// 同步通知地址


        $data['app_id'] = $payConf['business_num'];//商户号
        $data['money'] = sprintf('%.2f',$amount);//订单金额
        $data['mer_order'] = $order;//订单号
        $data['pay_type'] = $bank;//银行编码
        if($payConf['pay_way'] == 1){
            $data['pay_type'] = 'bank';
        }
        $data['syn_url'] = $ServerUrl;
        $data['jump_url'] = $returnUrl;
        $data['client_ip'] = self::getIp();
        $data['product_name'] = 'test';
        $data['pay_mode'] = '0';

        $signStr =  self::getSignStr($data, true, true);
        $data['attach'] = 'test';
        if($payConf['pay_way'] == 1){
            $data['bank_type'] = $bank;
        }

        $postSignStr =  self::getSignStr($data, true, true);
        $post['sign'] = md5($signStr . "&key=" . $payConf['md5_private_key']);
        $post['req'] = base64_encode($postSignStr);

        unset($reqData);
        return $post;
    }

    /**
     * @param $type
     * @param $json
     * @param $payConf
     * @return bool
     */
    public static function SignOther($type, $json, $payConf)
    {
        $post = file_get_contents('php://input');
        $data = json_decode($post,true);
        $sign = $data['sign'];
        unset($data['sign']);
        unset($data['attach']);
        unset($data['bank_type']);
        $signStr =  self::getSignStr($data, true, true);
        $signTrue = md5($signStr . "&key=" . $payConf['md5_private_key']);
        if (strtolower($sign) == strtolower($signTrue) && $data['status'] == '1') {
            return true;
        } else {
            return false;
        }
    }

}