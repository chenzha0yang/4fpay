...
class Baochengzfpay extends ApiModel
{
    ...
    public static function getAllInfo($reqData, $payConf)
    {
        ...
        //TODO: do something
        if((int)$payConf['pay_way'] === 0 || (int)$payConf['pay_way'] === 9)
        {
            $responseMode = 'jump_page';
        }else{
            $responseMode = 'pay_str';
        }
        ...
        $data['pay_format'] = $responseMode;
        $data['pay_clientip'] = self::getIp();
        $signStr =  self::getSignStr($data,true,true);
        $data['pay_md5sign'] = strtoupper(md5($signStr . "&key=" . $payConf['md5_private_key']));

        if($responseMode == 'pay_str'){
            self::$reqType = 'curl';
    ...
    public static function SignOther($type, $data, $payConf)
    {
        ...
        if (strtoupper($sign) == strtoupper($signTrue) && $data['returncode'] == '0000') {
            return true;
        }
        return false;
    }
