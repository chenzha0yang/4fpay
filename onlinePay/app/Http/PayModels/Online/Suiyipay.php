...
class Suiyipay extends ApiModel
{
    ...
    public static function getAllInfo($reqData, $payConf)
    {
        ...
        $data['orderid']     = $order;
        $data['callbackurl'] = $ServerUrl; //商户订单号
        $strToSign           = self::getSignStr($data);
        $data['sign']        = self::getMd5Sign($strToSign, $payConf['md5_private_key']);

        if ($payConf['pay_type'] == 456) {
            $data['payerIp'] = '127.0.0.1';
            $data['attach']  = '';
        }
        if ($payConf['pay_type'] == 458) {
            $data['attach'] = 'protect';
        }
        $data['hrefbackurl'] = $returnUrl; //商户同步通知地址
        unset($reqData);
        return $data;
    }
}