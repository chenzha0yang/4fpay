...
class Bangyinpay extends ApiModel
{
    ...
    public static function getAllInfo($reqData, $payConf)
    {
        ...
        self::$unit         = 2;
        $data               = [];
        $data['merId']      = $payConf['business_num'];//测试商户号
        $data['merOrderId'] = $order;//商户系统内部的订单号
        if ($payConf['pay_way'] == '1') {
            $data['paymentType'] = 'gate_web';
        } else {
            $data['paymentType'] = $bank;
        }
        $data['transAmt']   = $amount * 100;
