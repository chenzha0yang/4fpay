...
class Zfthpay extends ApiModel
{
    ...
    public static function getAllInfo($reqData, $payConf)
    {
        ...
        $data['requestTime'] = date('YmdHis', time());
        $data['syncUrl']     = $returnUrl;
        $data['asyncUrl']    = $ServerUrl;
        $signStr             = self::getSignStr($data, true, true);
        $data['sign']        = strtolower(md5($signStr . "&secretKey=" . $payConf['md5_private_key']));
        unset($reqData);
        return $data;
    }
    ...
    public static function SignOther($model, $data, $payConf)
    {
        ...
        unset($data['sign']);
        $signStr = self::getSignStr($data, true, true);
        $mySign  = strtolower(md5($signStr . "&secretKey=" . $payConf['md5_private_key']));
