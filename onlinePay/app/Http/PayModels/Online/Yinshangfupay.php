...
class Yinshangfupay extends ApiModel
{
    ...
    public static function SignOther($model, $data, $payConf)
    {
        ...
        $signData = [
            ...
            'paysapi_id' => $res['paysapi_id'],        //服务器API接口返回的唯一支付编码ID
            'order_id'   => $res['order_id'],          //用户订单编号ID
            'is_type'    => $res['is_type'],           //支付类型
            'price'      => $res['price'],             //订单金额
            'real_price' => $res['real_price'],        //实际支付金额
            'mark'       => $res['mark'],              //此处填写产品名称，或充值，消费说明
            'code'       => $res['code']               //订单状态
        ];
        $signStr      = self::getSignStr($signData, true, true);
        $mySign = strtoupper(md5($signStr . '&key=' . $payConf['md5_private_key']));
        if ($sign == $mySign && $res['code'] == '1') {
            return true;
        } else {
            return false;
        }
