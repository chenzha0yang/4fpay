...
class Jiujzfpay extends ApiModel
{
    ...
    public static function getAllInfo($reqData, $payConf)
    {
        ...
        $data['payMoney']    = $amount * 100;
        $data['spbillIp']    = self::getIp();
    ...
    public static function getVerifyResult($request)
    {
        ...
        if (!$arr) {
            CallbackMsg::addDebugLogs([$json]);
        }
        if (isset($arr['data']['payMoney'])) {
            $arr['data']['payMoney'] = $arr['data']['payMoney'] / 100;
        }
        return $arr['data'];
    }
