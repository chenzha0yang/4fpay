...
class Haihaipay extends ApiModel
{
    ...
    public static function getAllInfo($reqData, $payConf)
    {
        ...
        $data['sign']            = strtolower(md5("merchantName={$data['merchantName']}&merchantCode={$data['merchantCode']}&merchantOrderNo={$data['merchantOrderNo']}&payPrice={$data['payPrice']}&payType={$data['payType']}&{$payConf['md5_private_key']}"));
        ...
        $post['queryUrl'] = $reqData['formUrl'] . '/hhf-restful/server/create/pay/order';
    ...
    {        $sign = $data['sign'];
        ...
        if (strtolower($sign) == strtolower($signTrue) && $data['noticeType'] == '1') {
            return true;
        }elseif($data['noticeType'] == '2'){
            echo 'success';die;
        }else{
            return false;
        };
