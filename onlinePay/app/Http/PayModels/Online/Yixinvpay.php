...
class Yixinvpay extends ApiModel
{
    ...
    public static function getAllInfo($reqData, $payConf)
    {
        ...
        $data                 = [];
        $data['mchntId']      = $payConf['business_num'];
        $data['mchntOrderId'] = $order;
        $data['orderSubject'] = 'ceshi';
        $data['bizType']      = $bank;
        $data['txnAmt']       = $amount * 100;
        $data['currency']     = 'CNY';
        $data['notifyUrl']    = $ServerUrl;
        $data['returnUrl']    = $returnUrl;
        $data['sendIp']       = self::getIp();
    ...
    public static function getVerifyResult($request, $mod)
    {
        $data                = $request->all();
        if ($data['success']) {
            $res['txnAmt']       = $data['txnAmt'] / 100;
            $res['mchntOrderId'] = $data['mchntOrderId'];
            return $res;
        } else {
            return $data;
        }
