...
class Pangpangfupay extends ApiModel
{
    ...
    public static function getAllInfo($reqData, $payConf)
    {
        ...
        $data['currentTime'] = date('YmdHis',time());
        ...
        if($data['returnUrl']){
            $signStr = $signStr.'&returnUrl='.$data['returnUrl'];
        }
    ...
    public static function SignOther( $type, $data, $payConf )
    {
        ...
        $signStr = $signStr."&systemNo=".$systemNo;

        $signTrue = md5($signStr.'#'.$payConf['md5_private_key']);

        if ( strtoupper($sign) == strtoupper($signTrue) && $data['payFlag'] == 2) {
            return true;
        } else {
            return false;
        }
