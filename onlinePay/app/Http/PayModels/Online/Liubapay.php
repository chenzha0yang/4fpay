...
class Liubapay extends ApiModel
{
    ...
    public static function getAllInfo($reqData, $payConf)
    {
        ...
        $data['product_id']     = $bank;//银行编码
        $data['return_url'] = $returnUrl;
        $data['notify_url']   = $ServerUrl;
        $data['subject']      = '68';
        $data['pay_amount']       = $amount * 100;//订单金额
    ...
    public static function getQrCode($response)
    {
        ...
        if ($data['code'] == 1) {
            $pay_extends = $data['data']['pay_extends'];
            if($pay_extends['pay_action'] == 'get'){
                self::$isAPP = true;
            }
            $data['payUrl'] = $pay_extends['pay_url'];
        }
        return $data;
    }
    ...
    {
        $arr = $request->all();
        if (isset($arr['transaction_money'])) {
