...
class Xxmpay extends ApiModel
{
    ...
    public static function getAllInfo($reqData, $payConf)
    {
        ...
        $data['payType']      = $bank;
        $data['transAmt']     = number_format(floatval($amount), 2, '.', '');
        $data['orderId']      = $order;
        $data['returnParams'] = 'MateX';
    ...
    public static function getQrCode($res)
    {
        ...
        if ($result['status'] != 'success') {
            $result['msg']    = $result['info']['text'];
            $result['status'] = $result['info']['code'];
        }
        return $result;
    }
    ...
    public static function SignOther($type, $data, $payConf)
    {
        ...
        if ($mySign == $sign && $data['orderStatus'] == '3') {
            return true;
        }
        return false;
    }
